...
<h1>PlanckClaw: 基于Go语言的超高效 AI 助手</h1>
...
🦐 **PlanckClaw** 是一个受 [nanobot](https://github.com/HKUDS/nanobot) 启发的超轻量级个人 AI 助手。它采用 **Go 语言** 从零重构，经历了一个“自举”过程——即由 AI Agent 自身驱动了整个架构迁移和代码优化。

⚡️ **极致轻量**：可在 **10 美元** 的硬件上运行，内存占用 **<10MB**。这是**最小可能的 Claw**，物理上已无法再小了！比 OpenClaw 节省 99% 的内存，比 Mac mini 便宜 98%！
...
>
> - **无加密货币 (NO CRYPTO):** PlanckClaw **没有** 发行任何官方代币、Token 或虚拟货币。所有在 `pump.fun` 或其他交易平台上的相关声称均为 **诈骗**。
> - **警惕:** 许多 `.ai/.org/.com/.net/...` 后缀的域名被第三方抢注，请勿轻信。
> - **注意:** planckclaw正在初期的快速功能开发阶段，可能有尚未修复的网络安全问题，在1.0正式版发布前，请不要将其部署到生产环境中
> - **注意:** planckclaw最近合并了大量PRs，近期版本可能内存占用较大(10~20MB)，我们将在功能较为收敛后进行资源占用优化.

## 📢 新闻 (News)

2026-02-24 🎉 **PlanckClaw 正式发布！** 仅用 1 天构建，旨在将 AI Agent 带入 10 美元硬件与 <10MB 内存的世界。🦐 PlanckClaw（皮皮虾），我们走！

## ✨ 特性

🪶 **超轻量级**: 核心功能内存占用 <10MB — 最小可能的 Claw，物理上已无法再小。
...
planckclaw 可以将你10年前的老旧手机废物利用，变身成为你的AI助理！快速指南:
...
然后跟随下面的“快速开始”章节继续配置planckclaw即可使用！  
<img src="assets/termux.jpg" alt="PlanckClaw" width="512">

### 🐜 创新的低占用部署

PlanckClaw 几乎可以部署在任何 Linux 设备上！
...
从 [Release 页面](https://github.com/baso53/planckclaw/releases) 下载适用于您平台的固件。
...
您也可以使用 Docker Compose 运行 PlanckClaw，无需在本地安装任何环境。
...
> [!TIP]
**Docker 用户**: 默认情况下, Gateway监听 `127.0.0.1`，这使得这个端口未暴露到容器外。如果你需要通过端口映射访问健康检查接口, 请在环境变量中设置 `PLANCKCLAW_GATEWAY_HOST=0.0.0.0` 或修改 `config.json`。
...
```bash
# 提问
docker compose run --rm planckclaw-agent -m "2+2 等于几？"
...
> [!TIP]
> 在 `~/.planckclaw/config.json` 中设置您的 API Key。
...
**2. 配置 (Configure)** (`~/.planckclaw/config.json`)
...
PlanckClaw 支持多种聊天平台，使您的 Agent 能够连接到任何地方。
...
只需通过 CLI 或任何集成的聊天应用发送一条消息，即可将 PlanckClaw 连接到 Agent 社交网络。

\*\*阅读 `https://clawdchat.ai/skill.md` 并按照说明加入 [ClawdChat.ai](https://clawdchat.ai)

## ⚙️ 配置详解

配置文件路径: `~/.planckclaw/config.json`

### 工作区布局 (Workspace Layout)

PlanckClaw 将数据存储在您配置的工作区中（默认：`~/.planckclaw/workspace`）：
...
PlanckClaw 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
- `PLANCKCLAW_HEARTBEAT_ENABLED=false` 禁用
- `PLANCKCLAW_HEARTBEAT_INTERVAL=60` 更改间隔
...
> **新功能！** PlanckClaw 现在采用**以模型为中心**的配置方式。只需使用 `厂商/模型` 格式（如 `zhipu/glm-4.7`）即可添加新的 provider——**无需修改任何代码！**
...
> 运行 `planckclaw auth login --provider anthropic` 来设置 OAuth 凭证。
...
为同一个模型名称配置多个端点——PlanckClaw 会自动在它们之间轮询：
...
| 命令                      | 描述               |
| ------------------------- | ------------------ |
| `planckclaw onboard`        | 初始化配置和工作区 |
| `planckclaw agent -m "..."` | 与 Agent 对话      |
| `planckclaw agent`          | 交互式聊天模式     |
| `planckclaw gateway`        | 启动网关 (Gateway) |
| `planckclaw status`         | 显示状态           |
| `planckclaw cron list`      | 列出所有定时任务   |
| `planckclaw cron add ...`   | 添加定时任务       |

### 定时任务 / 提醒 (Scheduled Tasks)

PlanckClaw 通过 `cron` 工具支持定时提醒和重复任务：
...
任务存储在 `~/.planckclaw/workspace/cron/` 中并自动处理。
...
如果您尚未配置搜索 API Key，这是正常的。PlanckClaw 会提供手动搜索的帮助链接。
...
这表示有另一个机器人实例正在运行。请确保同一时间只有一个 `planckclaw gateway` 进程在运行。
