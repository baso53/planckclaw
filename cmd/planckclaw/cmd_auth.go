...
import (
	...
	"github.com/baso53/planckclaw/pkg/auth"
...
func authHelp() {
	...
	fmt.Println("  planckclaw auth login --provider openai")
	fmt.Println("  planckclaw auth login --provider openai --device-code")
	fmt.Println("  planckclaw auth login --provider anthropic")
	fmt.Println("  planckclaw auth login --provider google-antigravity")
	fmt.Println("  planckclaw auth models")
	fmt.Println("  planckclaw auth logout --provider openai")
	fmt.Println("  planckclaw auth status")
}
...
func authLoginGoogleAntigravity() {
	...
	fmt.Println("Try it: planckclaw agent -m \"Hello world\"")
}
...
func authStatusCmd() {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: planckclaw auth login --provider <name>")
		return
	}
...
func authModelsCmd() {
	...
	if err != nil || cred == nil {
		...
		fmt.Println("Run: planckclaw auth login --provider google-antigravity")
		return
	}
