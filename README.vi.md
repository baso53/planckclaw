...
<h1>PlanckClaw: Trợ lý AI Siêu Nhẹ viết bằng Go</h1>
...
🦐 **PlanckClaw** là trợ lý AI cá nhân siêu nhẹ, lấy cảm hứng từ [nanobot](https://github.com/HKUDS/nanobot), được viết lại hoàn toàn bằng **Go** thông qua quá trình "tự khởi tạo" (self-bootstrapping) — nơi chính AI Agent đã tự dẫn dắt toàn bộ quá trình chuyển đổi kiến trúc và tối ưu hóa mã nguồn.

⚡️ **Cực kỳ nhẹ:** Claw nhỏ nhất có thể — không thể nhỏ hơn về mặt vật lý. Chạy trên phần cứng chỉ **$10** với RAM **<10MB**. Tiết kiệm 99% bộ nhớ so với OpenClaw và rẻ hơn 98% so với Mac mini!
...
>
> * **KHÔNG CÓ CRYPTO:** PlanckClaw **KHÔNG** có bất kỳ token/coin chính thức nào. Mọi thông tin trên `pump.fun` hoặc các sàn giao dịch khác đều là **LỪA ĐẢO**.
> * **Cảnh báo:** Nhiều tên miền `.ai/.org/.com/.net/...` đã bị bên thứ ba đăng ký, không phải của chúng tôi.
> * **Cảnh báo:** PlanckClaw đang trong giai đoạn phát triển sớm và có thể còn các vấn đề bảo mật mạng chưa được giải quyết. Không nên triển khai lên môi trường production trước phiên bản v1.0.
> * **Lưu ý:** PlanckClaw gần đây đã merge nhiều PR, dẫn đến bộ nhớ sử dụng có thể lớn hơn (10–20MB) ở các phiên bản mới nhất. Chúng tôi sẽ ưu tiên tối ưu tài nguyên khi bộ tính năng đã ổn định.


## 📢 Tin tức

2026-02-24 🎉 PlanckClaw chính thức ra mắt! Được xây dựng trong 1 ngày để mang AI Agent đến phần cứng $10 với RAM <10MB. 🦐 PlanckClaw, Lên Đường!

## ✨ Tính năng nổi bật

🪶 **Siêu nhẹ**: Bộ nhớ sử dụng <10MB — Claw nhỏ nhất có thể, không thể nhỏ hơn về mặt vật lý.
...
PlanckClaw có thể triển khai trên hầu hết mọi thiết bị Linux!
...
Tải file binary cho nền tảng của bạn từ [trang Release](https://github.com/baso53/planckclaw/releases).
...
Bạn cũng có thể chạy PlanckClaw bằng Docker Compose mà không cần cài đặt gì trên máy.
...
> [!TIP]
> **Người dùng Docker**: Theo mặc định, Gateway lắng nghe trên `127.0.0.1`, không thể truy cập từ máy chủ. Nếu bạn cần truy cập các endpoint kiểm tra sức khỏe hoặc mở cổng, hãy đặt `PLANCKCLAW_GATEWAY_HOST=0.0.0.0` trong môi trường của bạn hoặc cập nhật `config.json`.
...
```bash
# Đặt câu hỏi
docker compose run --rm planckclaw-agent -m "2+2 bằng mấy?"
...
> [!TIP]
> Thiết lập API key trong `~/.planckclaw/config.json`.
...
**2. Cấu hình** (`~/.planckclaw/config.json`)
...
```bash
planckclaw agent -m "Xin chào, bạn là ai?"
```
...
Trò chuyện với PlanckClaw qua Telegram, Discord, DingTalk, LINE hoặc WeCom.
...
> **Docker Compose**: Thêm `ports: ["18791:18791"]` vào service `planckclaw-gateway` để mở port webhook.
...
PlanckClaw hỗ trợ hai loại tích hợp WeCom:
...
Kết nối PlanckClaw với Mạng xã hội Agent chỉ bằng cách gửi một tin nhắn qua CLI hoặc bất kỳ ứng dụng Chat nào đã tích hợp.

**Đọc `https://clawdchat.ai/skill.md` và làm theo hướng dẫn để tham gia [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Cấu hình chi tiết

File cấu hình: `~/.planckclaw/config.json`

### Cấu trúc Workspace

PlanckClaw lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.planckclaw/workspace`):
...
PlanckClaw chạy trong môi trường sandbox theo mặc định. Agent chỉ có thể truy cập file và thực thi lệnh trong phạm vi workspace.
...
|----------|---------|-------|
| `workspace` | `~/.planckclaw/workspace` | Thư mục làm việc của agent |
...
PlanckClaw có thể tự động thực hiện các tác vụ định kỳ. Tạo file `HEARTBEAT.md` trong workspace:
...
* `PLANCKCLAW_HEARTBEAT_ENABLED=false` để tắt
* `PLANCKCLAW_HEARTBEAT_INTERVAL=60` để thay đổi khoảng thời gian
...
> **Tính năng mới!** PlanckClaw hiện sử dụng phương pháp cấu hình **đặt mô hình vào trung tâm**. Chỉ cần chỉ định dạng `nhà cung cấp/mô hình` (ví dụ: `zhipu/glm-4.7`) để thêm nhà cung cấp mới—**không cần thay đổi mã!**
...
```
> Chạy `planckclaw auth login --provider anthropic` để thiết lập thông tin xác thực OAuth.

#### Cân bằng Tải tải

Định cấu hình nhiều endpoint cho cùng một tên mô hình—PlanckClaw sẽ tự động phân phối round-robin giữa chúng:
...
| Lệnh | Mô tả |
| --- | --- |
| `planckclaw onboard` | Khởi tạo cấu hình & workspace |
| `planckclaw agent -m "..."` | Trò chuyện với agent |
| `planckclaw agent` | Chế độ chat tương tác |
| `planckclaw gateway` | Khởi động gateway (cho bot chat) |
| `planckclaw status` | Hiển thị trạng thái |
| `planckclaw cron list` | Liệt kê tất cả tác vụ định kỳ |
| `planckclaw cron add ...` | Thêm tác vụ định kỳ |

### Tác vụ định kỳ / Nhắc nhở

PlanckClaw hỗ trợ nhắc nhở theo lịch và tác vụ lặp lại thông qua công cụ `cron`:
...
Các tác vụ được lưu trong `~/.planckclaw/workspace/cron/` và được xử lý tự động.
...
Điều này là bình thường nếu bạn chưa cấu hình API key cho tìm kiếm. PlanckClaw sẽ cung cấp các liên kết hữu ích để tìm kiếm thủ công.
...
Thêm key vào `~/.planckclaw/config.json` nếu dùng Brave:
...
Điều này xảy ra khi có một instance bot khác đang chạy. Đảm bảo chỉ có một tiến trình `planckclaw gateway` chạy tại một thời điểm.
