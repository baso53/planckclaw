...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in PlanckClaw.
...
10. [Creating a New Provider in PlanckClaw](#creating-a-new-provider-in-planckclaw)
...
const antigravityPlugin = {
  ...
  register(api: PlanckClawPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<PlanckClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- PlanckClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.planckclaw/auth.json`:
...
## Creating a New Provider in PlanckClaw

PlanckClaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/planckclaw/cmd_auth.go`:
...
planckclaw auth login --provider your-provider

# List models (for Antigravity)
planckclaw auth models

# Start the gateway
planckclaw gateway

# Run an agent with a specific model
planckclaw agent -m "Hello" --model your-model
```
...
export PLANCKCLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export PLANCKCLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `planckclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.planckclaw/auth.json`
- Re-run `planckclaw auth login --provider antigravity`
