...
func Run(opts Options) (*Result, error) {
	...
	planckClawHome, err := resolvePlanckClawHome(opts.PlanckClawHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, planckClawHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to PlanckClaw")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, planckClawHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, planckClawHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(planckClawHome, "config.json"),
				Description: "convert OpenClaw config to PlanckClaw format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(planckClawHome)
...
func Execute(actions []Action, openclawHome, planckClawHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, planckClawHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, planckClawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing PlanckClaw config: %w", err)
		}
...
func resolvePlanckClawHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("PLANCKCLAW_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".planckclaw"), nil
}
