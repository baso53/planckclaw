...
func TestConvertConfig(t *testing.T) {
	...
	t.Run("unsupported provider warning", func(t *testing.T) {
		...
		if warnings[0] != "Provider 'unknown_provider' not supported in PlanckClaw, skipping" {
	...
	t.Run("unsupported channel warning", func(t *testing.T) {
		...
		if warnings[0] != "Channel 'email' not supported in PlanckClaw, skipping" {
			t.Errorf("unexpected warning: %s", warnings[0])
		}
	})
	...
	t.Run("agent defaults", func(t *testing.T) {
		...
		if cfg.Agents.Defaults.Workspace != "~/.planckclaw/workspace" {
			t.Errorf("Workspace = %q, want %q", cfg.Agents.Defaults.Workspace, "~/.planckclaw/workspace")
		}
	})
...
func TestRewriteWorkspacePath(t *testing.T) {
	tests := []struct {
		...
		{"default path", "~/.openclaw/workspace", "~/.planckclaw/workspace"},
...
func TestRunDryRun(t *testing.T) {
	...
	planckWs := filepath.Join(planckClawHome, "workspace")
	if _, err := os.Stat(filepath.Join(planckWs, "SOUL.md")); !os.IsNotExist(err) {
		t.Error("dry run should not create files")
	}
	if _, err := os.Stat(filepath.Join(planckClawHome, "config.json")); !os.IsNotExist(err) {
...
func TestRunFullMigration(t *testing.T) {
	...
	soulData, err := os.ReadFile(filepath.Join(planckWs, "SOUL.md"))
	if err != nil {
	...
	agentsData, err := os.ReadFile(filepath.Join(planckWs, "AGENTS.md"))
	if err != nil {
	...
	memData, err := os.ReadFile(filepath.Join(planckWs, "memory", "MEMORY.md"))
	if err != nil {
	...
	planckConfig, err := config.LoadConfig(filepath.Join(planckClawHome, "config.json"))
	if err != nil {
		t.Fatalf("loading PlanckClaw config: %v", err)
	}
	if planckConfig.Providers.Anthropic.APIKey != "sk-ant-migrate-test" {
		t.Errorf("Anthropic.APIKey = %q, want %q", planckConfig.Providers.Anthropic.APIKey, "sk-ant-migrate-test")
	}
	if planckConfig.Providers.OpenRouter.APIKey != "sk-or-migrate-test" {
		t.Errorf("OpenRouter.APIKey = %q, want %q", planckConfig.Providers.OpenRouter.APIKey, "sk-or-migrate-test")
	}
	if !planckConfig.Channels.Telegram.Enabled {
		t.Error("Telegram should be enabled")
	}
	if planckConfig.Channels.Telegram.Token != "tg-migrate-test" {
		t.Errorf("Telegram.Token = %q, want %q", planckConfig.Channels.Telegram.Token, "tg-migrate-test")
	}
